...
class couchClient extends couch {
	...
	*/
	...
	/**
	* @var array Lucene query options definitions
	* See https://github.com/rnewson/couchdb-lucene for information on couchdb-lucene
	*
	* key is the couchClient method (mapped with __call)
	* value is a hash containing :
	*	- name : the query option name (couchdb side)
	*	- filter : the type of filter to apply to the value (ex to force a cast to an integer ...)
	*/
	protected $lucene_query_defs = array (
		
		"callback" 				=> array ("name" => "callback", "filter"=> "string"),
		"debug" 				=> array ("name" => "debug", "filter"=> "jsonEncodeBoolean"),
		"default_operator" 		=> array ("name" => "default_operator", "filter"=> "string"),
		"force_json" 			=> array ("name" => "force_json", "filter"=> "jsonEncodeBoolean"),
		"include_docs" 			=> array ("name" => "include_docs", "filter"=> "jsonEncodeBoolean"),
		"include_fields" 		=> array ("name" => "include_fields", "filter"=> "jsonEncodeBoolean"),
		"limit"					=> array ("name" => "limit", "filter"=> "int"),
		"skip"					=> array ("name" => "skip", "filter"=> "int"),
		"sort" 					=> array ("name" => "sort", "filter"=> "string"),
		"stale" 			=> array ("name" => "stale", "filter"=>"staticValue", "staticValue"=> "ok"),
	...
	*/
	...
	function __call($name, $args) {	
		...
		if ( array_key_exists($name,$this->query_defs))
			$query_defs = $this->query_defs;
		else if(array_key_exists($name,$this->lucene_query_defs))
			$query_defs = $this->lucene_query_defs;
		else
			throw new Exception("Method $name does not exist");
		
		if ( $query_defs[$name]['filter'] == 'int' ) {
			$this->query_parameters[ $query_defs[$name]['name'] ] = (int)reset($args);
		} elseif ( $query_defs[$name]['filter'] == 'staticValue' ) {
			$this->query_parameters[ $query_defs[$name]['name'] ] = $query_defs[$name]['staticValue'];
		} elseif ( $query_defs[$name]['filter'] == 'jsonEncode' ) {
			$this->query_parameters[ $query_defs[$name]['name'] ] = json_encode(reset($args));
		} elseif ( $query_defs[$name]['filter'] == 'ensureArray' ) {
			if ( is_array(reset($args)) ) {
				$this->query_parameters[ $query_defs[$name]['name'] ] = reset($args);
			}
		} elseif ( $query_defs[$name]['filter'] == 'string' ) {
			$this->query_parameters[ $query_defs[$name]['name'] ] = (string)reset($args);
		} elseif ( $query_defs[$name]['filter'] == 'jsonEncodeBoolean' ) {
			$this->query_parameters[ $query_defs[$name]['name'] ] = json_encode((boolean)reset($args));
		} else {
			$this->query_parameters[ $query_defs[$name]['name'] ] = reset($args);
		}
// 		print_r($this->query_parameters);
	...
	/**
        * Set all Lucene query options at once.
        * Any invalid options are ignored.
		* See https://github.com/rnewson/couchdb-lucene for information on couchdb-lucene
        *
        * @link http://wiki.apache.org/couchdb/HTTP_view_API
        ...
        * @return couchClient $this
        */
	public function setLuceneQueryParameters(array $options) {
          foreach($options as $option=>$v) if (array_key_exists($option,$this->lucene_query_defs))
              $this->$option($v);
              return $this;
	}
...
// 			return $this->_continuousChanges();
	...
	*/
	public function getLuceneView ( $id, $name, $query=null ) {
		if ( !$id OR !$name) throw new InvalidArgumentException("You should at least specify design doc id and lucene view name");
