...
if(!empty($cmd)) {
	if($cmd == 'query_view') {
		...
		try {
			$res = $cdb[$cdb_inx]->setQueryParameters($opts)->getView($postf, $view);
		}
		catch(Exception $e) {
		...
		try {	
			...
			$res = $cdb[$cdb_inx]->setQueryParameters(array('endkey'=>new stdClass(), 'limit'=>300, 'reduce'=>false, 'include_docs'=>false))
						->getView($postf, $view);
